...
"	FROM anamnesis an join post_parto pp using(post_parto_id) join nacimiento n    using (nacimiento_codigo) join detalle_nacimiento dn using (nacimiento_codigo) where an.hijo_codigo="+ FiltroHijosVictima.getCodigo() +";";
    System.out.println(sql);
        try {
            rs = conectar.query(sql);
            while (rs.next()) {
